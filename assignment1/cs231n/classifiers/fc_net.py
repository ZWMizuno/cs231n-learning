...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # If y is None then we are in test mode so just return scores测试环节，只需返回scores
        if y is None:
            return scores
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss, dsm = softmax_loss(A2_out, y)
        loss += 0.5 * self.reg * (np.sum(self.params['W2'] * self.params['W2']) + np.sum(self.params['W1'] * self.params['W1']))#正则化涵盖所有权重
